...
def execute(filters=None):
	columns = [
		...
		{'fieldname': 'tax_id', 'label': _('VAT/PAN Number'), 'fieldtype': 'Data'},
	...
	for land in landed_cost:
		...
		for item in items:
			for r in receipts:
				tax_id = frappe.db.get_value("Supplier", r.supplier, "tax_id")
				grand_totals = 0
			for t in taxes: 
				data.append([land.nepali_date, land.name, item.receipt_document_type, item.receipt_document, item.item_code, item.description, item.qty, item.rate, item.amount, item.applicable_charges, r.supplier, tax_id, t.expense_account, land.distribute_charges_based_on, land.total_taxes_and_charges])
	return columns, data